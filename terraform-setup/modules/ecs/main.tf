...
resource "aws_cloudwatch_log_group" "demo" {
  name = "ascent-demo-log-group"

  tags = {
    Environment = "${var.environment}"
    Application = "AscentDemo"
  }
}

/* ==== ECR repository to store our Docker images ==== */
data "aws_ecr_repository" "demo-repo" {
...
/* task definition for the web service (Flask application) */
...
resource "aws_alb" "alb_ascent-demo" {
...
resource "aws_alb_listener" "ascent-demo-listener" {
  load_balancer_arn = "${aws_alb.alb_ascent-demo.arn}"
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.environment}_ascent-demo_web_cpu_utilization_high"
  ...
  dimensions = {
    ...
    ServiceName = "${aws_ecs_service.ascent-demo.name}"
  }
